...
public class ComputeTexture : MonoBehaviour {
	...
	protected Texture2D ConvertFromRenderTexture(RenderTexture rt, TextureFormat textureFormat){
		Texture2D output = new Texture2D(squareResolution, squareResolution, textureFormat, true);
	...
	public virtual void SaveAsset(){
		...
		Texture2D output = ConvertFromRenderTexture(rwTexture.rt, assetTextureFormat);
		AssetDatabase.CreateAsset(output, path);
        Debug.Log("Wrote 2D Texture "+output+" to "+path);
	}
